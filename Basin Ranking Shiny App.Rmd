...
ui <- fluidPage(
  titlePanel("SAP Basin Ranking Application"),
  sidebarLayout(
    sidebarPanel(
...
server <- function(input, output, session) {
  ...
  for (col in cols) {
    ...
    # Default to 1 if NULL, NA or empty
  ...
  # Default breaks as equal thirds of the range of scores
  ...
  # UI sliders for break adjustments with default values
  output$break_sliders_ui <- renderUI({
    req(default_breaks())
    breaks <- default_breaks()
    tagList(
      sliderInput("break1", "Lower Break (Low/Medium):",
                  min = floor(breaks[1]),
                  max = ceiling(breaks[4]),
                  value = breaks[2], step = 0.5),
      sliderInput("break2", "Upper Break (Medium/High):",
                  min = floor(breaks[1]),
                  max = ceiling(breaks[4]),
